      ...
      <div class="container-fluid">
        <div class="card">
          <div class="card-header">
            <a href="{{ route('info_create') }}" class="btn btn-sm btn-primary float-right btn-flat">Tambah Infografis</a>
          </div>
          <!-- /.card-header -->
          <div class="card-body">
            <table id="halaman" class="table table-bordered table-striped">
              ...
              <tbody>
                ...
                <tr>
                  <td>{{ $i }}</td>
                  <td>{{ $item->title }}</td>
                  <td><img src="{{ asset('images/info/'.$item->pic) }}" width="200"></td>
                  <td>
                    <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('info_destroy', $item->id) }}" method="POST">
                      <a href="{{ route('info_edit', $item->id) }}" class="btn btn-xs btn-primary">Edit</a>
                      @csrf
                      @method('DELETE')
